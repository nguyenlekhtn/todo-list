...
const defaultDate = formatISO(addDays(new Date(), 1), {
  representation: "date",
});
...
const projectListView = (() => {
  ...
  pubsub.subscribe("projectInfoChanged", ({ id, name, description }, topic) => {
    const changedProjectDOM = document.querySelector(
      `div[data-projectid='${id}']`
    );
  ...
  document.forms["new-project-form"].addEventListener("submit", (e) => {
    e.preventDefault();
    const name =
      document.forms["new-project-form"].elements["new-project-name"].value;
    const description =
      document.forms["new-project-form"].elements["new-project-description"]
        .value;
...
const projectView = (() => {
  ...
  // edit project info
  document
    .querySelector(".project-edit-icon")
    .addEventListener("click", (e) => {
      document.getElementById("edit-input-name").value =
        projectNameDOM.textContent;
      document.getElementById("edit-input-description").value =
        projectDescDOM.textContent;
      toggleProjectEditDisplay();
    });

  // delete project
  document
    .querySelector(".project-trash-icon")
    .addEventListener("click", (e) => {
  ...
  // cancel
  document
    .getElementById("cancel-project-info-btn")
    .addEventListener("click", (e) => {
      toggleProjectEditDisplay();
    });
  ...
  // submit project info
  document
    .getElementById("submit-project-info-btn")
    .addEventListener("click", (e) => {
      e.preventDefault();
      const name = document.getElementById("edit-input-name").value;
      const description = document.getElementById("edit-input-description")
        .value;
  ...
  function toggleProjectEditDisplay() {
    [editContainer.style.display, infoContainer.style.display] = [
      window.getComputedStyle(infoContainer, null).display,
      window.getComputedStyle(editContainer, null).display,
    ];
  }
})();
...
const TaskDOM = (task) => {
  ...
  submitButton.addEventListener("click", (e) => {
    ...
    pubsub.publish("taskInfoSubmitted", {
      task,
      title: newTaskTitle,
  ...
  function _changeTaskDisplay() {
    [editTaskDOM.style.display, taskContentDOM.style.display] = [
      window.getComputedStyle(taskContentDOM, null).display,
      window.getComputedStyle(editTaskDOM, null).display,
    ];
  }
...
const TaskListView = (() => {
  const taskListDOM = document.querySelector(
    "#container > div.main-panel > div.tasks-container"
  );
